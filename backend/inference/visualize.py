...
def visualize_results(image, boxes, masks, category, scores, save_path=None):
    ...
    masks: list of HxW binary masks OR None
    category: list[str] or single str
    ...
    for i, box in enumerate(boxes):
        ...
        # category[i] if list, otherwise category
        label = category[i] if isinstance(category, list) else category
        score_text = f"{label} {scores[i]:.2f}" if scores is not None else label

        cv2.rectangle(image_vis, (x_min, y_min), (x_max, y_max), (0, 255, 0), 1)
        cv2.putText(
            image_vis, score_text,
            (x_min, y_min - 5),
    ...
    if masks is not None and len(masks) > 0:
        ...
        for i, mask in enumerate(masks):
            color = (np.array(cmap(i / max(num_masks, 1))[:3]) * 255).astype(np.uint8)
            color_mask[mask == 1] = color
